<div>
    <div class="card mb-4 box-shadow">
        ...
        <table class="table table-sm table-bordered table-striped  text-sm mb-3"
        style="white-space: nowrap;" wire:poll.10000ms>
            <thead>
                <tr>
                    ...
                    <tr style="background-color: {{ $bg }}">
                        ...
                        <td class="text-center d-flex justify-content-center">
                            ...
                            <button wire:click="panggilLog('{{$item->no_rawat}}','{{ $item->kd_dokter }}', '{{$kd_ruang_poli}}', '{{$item->no_reg}}', '{{$kd_display}}')"
                                class="btn btn-xs btn-primary" role="button" aria-disabled="true"><i class="fas fa-bullhorn"></i> Panggil</button>
                            <button wire:click="handleLog('{{ $item->kd_dokter }}', '{{ $item->no_rawat }}', '{{ $kd_ruang_poli }}', 'ada')"
                                class="btn btn-xs btn-success ml-2" aria-disabled="true"><i class="fas fa-check"></i> Ada</button>
                            <button wire:click="handleLog('{{ $item->kd_dokter }}', '{{ $item->no_rawat }}', '{{ $kd_ruang_poli }}', 'tidakada')"
                                class="btn btn-xs btn-danger mx-2" aria-disabled="true"><i class="fas fa-ban"></i> Tidak Ada</button>
                            <button wire:click="resetLog('{{ $item->no_rawat }}')"
                                class="btn btn-xs btn-default" aria-disabled="true"><i class="fas fa-undo"></i> Ulang</button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
