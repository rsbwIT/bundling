    ...
    @endif
            ...
            <form method="POST" action="{{ route('bpjs.inacbg-ujicoba.simpan') }}">
                ...
                <div class="btn-area d-flex flex-wrap" style="gap: 10px;">
                    ...
                    <!-- Tombol Print Klaim -->
                    @if($status_kirim)
                        <a href="{{ url('/inacbg-ujicoba/print/'.$nosep) }}" target="_blank" class="btn-eklaim btn-danger d-flex align-items-center">
...
function simpanResume() {
    ...
    $.ajax({
        url: '{{ route("inacbg-ujicoba.updateResumeData") }}',
        type: 'POST',
        data: data,
...
function simpanTriase() {
    ...
    $.ajax({
        url: '{{ route("inacbg-ujicoba.updateTriaseData") }}',
        type: 'POST',
        data: data,
        success: function(response) {
            btn.innerHTML = originalText;
...
<script>
    ...
    function validasiSITB() {
        ...
        fetch('{{ route("bpjs.inacbg-ujicoba.sitbValidate") }}', {
            method: 'POST',
            headers: {
    ...
    function batalValidasiSITB() {
        ...
        fetch('{{ route("bpjs.inacbg-ujicoba.sitbInvalidate") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
