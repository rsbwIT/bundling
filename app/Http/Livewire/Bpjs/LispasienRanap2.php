...
class LispasienRanap2 extends Component
{
    public $tanggal1;
    public $tanggal2;
    public $penjamnin;
    ...
    {
        return view('livewire.bpjs.lispasien-ranap2', [
            'getPasien' => $this->getListPasienRanap()
        ]);
    }
    ...
    public $nm_pasien;
    use WithFileUploads;
    public function SetmodalInacbg($no_rawat, $no_rkm_medis, $nm_pasien)
    {
    ...
    public function UploadInacbg($no_rawat, $no_rkm_medis)
    {
        try {
            if (!$this->upload_file_inacbg || !$this->upload_file_inacbg->isValid()) {
                session()->flash('errorBundling', 'Gagal!! File tidak ditemukan atau tidak valid.');
                return;
            }

            $no_rawatSTR = str_replace('/', '', $no_rawat);
            $extension   = $this->upload_file_inacbg->getClientOriginalExtension();
            ...
            // Simpan file baru
            $this->upload_file_inacbg->storeAs('file_inacbg', $file_name, 'public');
    ...
    public function SetmodalScan($no_rawat, $no_rkm_medis, $nm_pasien)
    {
        $this->no_rawat = $no_rawat;
        $this->no_rkm_medis = $no_rkm_medis;
        $this->nm_pasien = $nm_pasien;
    ...
    public function UploadScan($no_rawat, $no_rkm_medis)
    {
        $file = $this->upload_file_scan ?? null;
        $kode = $this->kode_berkas ?? null;
        ...
        try {
            ...
            // Reset input
            $this->upload_file_scan = null;
            $this->kode_berkas = null;
        } catch (\Throwable $th) {
