...
namespace App\Http\Controllers\DetailTindakanBPJS;
...
class OperasiAndVKBpjs extends Controller
{
    ...
    function OperasiAndVKBpjs(Request $request)
    {
        ...
        $OperasiAndVK = DB::table('operasi')
            ...
            ->where(function ($query) use ($kdPetugas, $kdDokter, $status, $tanggl1, $tanggl2) {
                if ($kdPetugas) {
        ...
        // Hitung jumlah kemunculan setiap nomor rawat
        $rawatCounts = array_count_values($OperasiAndVK->pluck('no_rawat')->toArray());

        $OperasiAndVK->map(function ($item) use ($rawatCounts) {
            $item->is_double = ($rawatCounts[$item->no_rawat] ?? 0) > 1;

            // Cek apakah ada COB dari detail_piutang_pasien
            $cobData = DB::table('detail_piutang_pasien')
                ->select('detail_piutang_pasien.kd_pj', 'penjab.png_jawab as nama_bayar', 'detail_piutang_pasien.totalpiutang')
                ->join('penjab', 'penjab.kd_pj', '=', 'detail_piutang_pasien.kd_pj')
                ->where('detail_piutang_pasien.no_rawat', $item->no_rawat)
                ->where('detail_piutang_pasien.kd_pj', '!=', 'BPJ')
                ->first();

            $item->cob_kd_pj = $cobData ? $cobData->kd_pj : null;
            $item->cob_nama_bayar = $cobData ? $cobData->nama_bayar : null;
            $item->cob_totalpiutang = $cobData ? $cobData->totalpiutang : null;

            // REGISTRASI
            $item->getRegistrasi = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Registrasi')->get();
            // Obat+Emb+Tsl / OBAT
            $item->getObat = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Obat')->get();
            // Retur Obat
            $item->getReturObat = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Retur Obat')->get();
            // Resep Pulang
            $item->getResepPulang = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Resep Pulang')->get();
            // RALAN DOKTER
            $item->getRalanDokter = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Ralan Dokter')->get();
            // RALAN DOKTER PARAMEDIS
            $item->getRalanDrParamedis = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Ralan Dokter Paramedis')->get();
            // RALAN PARAMEDIS
            $item->getRalanParamedis = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Ralan Paramedis')->get();
            // RANAP DOKTER
            $item->getRanapDokter = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Ranap Dokter')->get();
            // RANAP DOKTER PARAMEDIS
            $item->getRanapDrParamedis = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Ranap Dokter Paramedis')->get();
            // RANAP PARAMEDIS
            $item->getRanapParamedis = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Ranap Paramedis')->get();
            // OPRASI
            $item->getOprasi = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Operasi')->get();
            // LABORAT
            $item->getLaborat = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Laborat')->get();
            // RADIOLOGI
            $item->getRadiologi = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Radiologi')->get();
            // TAMBAHAN
            $item->getTambahan = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Tambahan')->get();
            // POTONGAN
            $item->getPotongan = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Potongan')->get();
            // KAMAR INAP
            $item->getKamarInap = DB::table('billing')->select('totalbiaya')->where('no_rawat', $item->no_rawat)->where('status', '=', 'Kamar')->get();
