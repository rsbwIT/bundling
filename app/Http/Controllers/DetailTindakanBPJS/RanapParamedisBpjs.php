...
class RanapParamedisBpjs extends Controller
{
    ...
    function RanapParamedisBpjs(Request $request)
    {
        $action = '/ranap-paramedis-bpjs';
        ...
        $getRanapParamedis = DB::table('pasien')
            ...
            ->where(function ($query) use ($kdPetugas, $status, $tanggl1, $tanggl2) {
                if ($kdPetugas) {
        ...
        $RalanParamedis = DB::table('pasien')
            ->select(
                ...
                'rawat_jl_pr.bhp',
                DB::raw("CASE WHEN rawat_jl_pr.kd_jenis_prw IN ('RJBP0053', 'RJBP0056') THEN jns_perawatan.tarif_tindakanpr ELSE rawat_jl_pr.tarif_tindakanpr END as tarif_tindakanpr"),
                'rawat_jl_pr.kso',
